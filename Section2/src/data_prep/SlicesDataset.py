...
class SlicesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #sample['image'] = torch.from_numpy(self.data[slc[0]]['image'][slc[1]][None, :]).cuda() 
        #sample['seg'] = torch.from_numpy(self.data[slc[0]]['seg'][slc[1]][None, :]).cuda()          
        
        sample["image"] = torch.from_numpy(self.data[slc[0]]["image"][slc[1]]).type(torch.cuda.FloatTensor).unsqueeze(0)                               
        sample["seg"] = torch.from_numpy(self.data[slc[0]]["seg"][slc[1]]).long().cuda().unsqueeze(0)       
        
        #sample['image'] = torch.from_numpy(self.data[slc[0]]['image'][slc[1]]).unsqueeze(0).cuda()
        #sample['seg'] = torch.from_numpy(self.data[slc[0]]['seg'][slc[1]][None, :]).long().cuda()        
