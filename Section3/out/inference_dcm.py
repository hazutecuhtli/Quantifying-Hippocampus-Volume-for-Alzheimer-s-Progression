...
def get_predicted_volumes(pred):
    ...
    print({"anterior": volume_ant, "posterior": volume_post, "total": total_volume})
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    suma = 0
    index = 0
    for kn in range(pred_vol.shape[0]):
        if pred_vol[kn,:,:].sum() > suma:
            suma = pred_vol[kn,:,:].sum()
            index = kn
    
    nd_orig = np.flip((orig_vol[index, :, :]/np.max(orig_vol[index, :, :]))*0xff).T.astype(np.uint8)
    # create a PIL image from numpy array
    pil_orig = Image.fromarray(nd_orig, mode="L").convert("RGBA").resize((450, 450))
    # paste the PIL image into the main report image object (pimg)
    pimg.paste(pil_orig, box=(0, 500))

    nd_pred = np.flip((pred_vol[index, :, :]/np.max(pred_vol[index, :, :]))*0xff).T.astype(np.uint8)
    # create a PIL image from numpy array
    pil_pred = Image.fromarray(nd_pred, mode="L").convert("RGBA").resize((450, 450))
    # paste the PIL image into the main report image object (pimg)
    pimg.paste(pil_pred, box=(500, 500))        
...
def get_series_for_inference(path):
    ...
    path = [os.path.join(path,folder) for folder in os.listdir(path) if 'HCropVolume' in folder][0]
